...
class VenueModel: NSObject {
    ...
    init?(venueDict: Dictionary<String, Any>) {
        ...
        for visitor in visitors{
            ...
            let visitorModel = VisitorModel(visitorId: visitorId, visitorName: visitorName, arriveTime: arriveTime, leaveTime: leaveTime)
            self.visitors.append(visitorModel)
        }
        
        // Sorted by arrival time
        self.visitors.sort(by: {
            return $0.arriveTime < $1.arriveTime ? true : false
        })
    ...
    func findAndappendIdleIntervalsIfAny(){
        // An array of tuple which will have intervals(VisitorModel) when there were no visitors, along with its index relative to visitors array.
        ...
        for index in 0..<visitors.count{
            ...
            if visitor.arriveTime > idleStartTime{
                let model = VisitorModel(visitorId: "Unknown", visitorName: "No Visitors", arriveTime: idleStartTime, leaveTime: visitor.arriveTime)
        ...
        if idleStartTime < closeTime{
            let model = VisitorModel(visitorId: "Unknown", visitorName: "No Visitors", arriveTime: idleStartTime, leaveTime: closeTime)
            model.isNoVisitor = true
